...
if ! grep -q '\[PASS\] AHB tohost signalled success' "$BUILD_DIR/${NAME}.ahb.sim.log"; then
  echo "AHB toolchain test did not report PASS; see $BUILD_DIR/${NAME}.ahb.sim.log" >&2
  exit 1
fi

echo "AHB toolchain test passed: $NAME"
echo "ELF:  $BUILD_DIR/${NAME}.elf"
echo "HEX:  $BUILD_DIR/${NAME}.hex"
echo "DUMP: $BUILD_DIR/${NAME}.dump"
echo "LOG:  $BUILD_DIR/${NAME}.ahb.sim.log"
if [[ "${RUN_FSDB:-}" == 1 ]]; then
